...
| --- | --- | --- |
| builder | **apex solar kiss** | native `/apex-solar-kiss/` (the object map) + GitHub `@apexSolarKiss` + native `/apex-solar-kiss/design/` (live design-system surfaces + the retained design portfolio board) |
...
Native repo surfaces (`/apex-solar-kiss/`, `/askproduktion/`, `/asymptotic-system-key/`) are the intended public canonical after the source-of-truth flip. **Notion is retained only as a legacy / staging / named-exception layer** — apex solar kiss keeps a retained **root** Notion exception page whose scope is the **portfolio / assets content** (design portfolio / apexSolarKiss assets), migration-pending — the **UI/UX** and **logo/branding comp boards are retired from public wayfinding** and are not migration candidates, so the design portfolio is the sole remaining apex solar kiss external-design exception; the builder **object map** is native at `/apex-solar-kiss/` and is not part of that exception. ASKproduKtion's Timberland films remain a named external-video exception on their own root Notion page. Asymptotic system key uses named external media sources (Lightroom stills + YouTube motion) through ASK-native facades. Notion is no longer the general embed/depth layer.
