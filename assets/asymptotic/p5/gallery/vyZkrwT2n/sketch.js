...
function setup() {
  ...
  for (let i = 0; i < 2; i++) {
    controls[i] = new Corner(
      controlsCenter.x + ((2 * i - 1) * stch0) / 2,
      controlsCenter.y + ((2 * i - 1) * stch0) / 3
    );
  }

  axisControl[0] = new Axis(controls[0], controls[1], 1);
  axisControl[1] = new Axis(controls[1], controls[0], 1);
...
function renderColors() {
  ...
  if ((colorBackground != color1) && (colorBackground != color2)) renderColors();
}
...
function draw() {
  ...
  for (let star of stars) {
    star.axis3.stch(invCosn(t)/100, invCosn(t)/160);
    star.show();
  }
...
/*    for (let i = 0; i < this.dim + 1; i++) {
    ...
    this.corner1.move(-_stch1, -_stch2);
    this.corner2.move(_stch1, _stch2);
  }
}
...
class Star {
  ...
  show() {
    for (let i = 0; i < this.dim + 1; i++) {
      let xP =
        this.axis3.corner1.x + (i * (this.axis3.corner2.x - this.axis3.corner1.x)) / this.dim;
      let yP =
        this.axis3.corner1.y + (i * (this.axis3.corner2.y - this.axis3.corner1.y)) / this.dim;
      this.axis3.notches[i] = { x: xP, y: yP };
    }
    for (let i = 0; i < this.dim + 1; i++) {
