...
This repo's `AGENTS.md` is a **resolved carrier**: the shared execution protocol (below, resolved from `control-surface/protocol/AGENTS.shared.md`) + the `core-ecology` profile + the ASK-local delta. The shared block is machine-maintained — do not hand-edit between the markers.
...
These rules are the repo-local delta for `ASK` (the live public front door at A-S-K.studio) on top of the shared protocol + `core-ecology` profile above. `ASK`'s local delta wins for `ASK` where an explicit conflict exists.
